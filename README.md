...
- 선언형 프로그래밍 도입: Suspense, ErrorBoundary
- Streaming SSR
- 웹소켓 BFF 구축과 클라이언트 통신 최적화

> **새롭게 알게 된 것**

- useSuspenseQuery를 SSR에서 사용하려면 데이터 프리페칭 필요: HydrationBoundary와 dehydrate를 통한 서버-클라이언트 상태 동기화의 중요성을 체감했습니다.
- WebSocket 연결 관리의 복잡성: 클라이언트 수에 따른 연결 풀 관리, 중복 방지, 순서 보장, 메모리 최적화 등 실시간 통신에서 고려해야 할 요소들이 생각보다 많았습니다.
- Context API의 최적화 포인트: useMemo와 useCallback을 활용한 Provider 최적화가 대규모 실시간 데이터 업데이트에서 얼마나 중요한지 알게 되었습니다.
- BFF 패턴의 실용성: 단순히 API 프록시를 넘어서 데이터 가공, 캐싱, 연결 관리까지 담당하는 BFF의 역할이 실시간 서비스에서 필수적임을 깨달았습니다.
- Streaming SSR의 실제 성능 차이: TTFB 개선과 점진적 렌더링이 사용자 체감 성능에 미치는 영향이 예상보다 컸습니다.
- zod의 강력함: 런타임 유효성 검사와 타입스크립트 타입 추론을 동시에 제공하는 zod의 효율성을 경험했습니다.

### 2. 앞으로 더 도전해 볼만한 것들

> **실시간 통신 고도화**

- WebRTC: P2P 실시간 통신으로 서버 부하 분산 및 지연시간 최소화

> **성능 및 확장성**

- Web Workers: 복잡한 계산을 백그라운드에서 처리하여 메인 스레드 부하 감소
- Service Workers: 오프라인 지원 및 백그라운드 동기화
