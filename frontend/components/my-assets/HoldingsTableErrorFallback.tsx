...
interface HoldingsTableErrorFallbackProps {
    error: Error;
    resetErrorBoundary: () => void;
}

export function HoldingsTableErrorFallback({ error, resetErrorBoundary }: HoldingsTableErrorFallbackProps) {
    const handleRetry = () => {
        console.error("HoldingsTable Error:", error);
        resetErrorBoundary();
    };
    ...
    return (
        <section className="contents-container flex flex-col gap-4 items-center justify-center py-8">
            <header>
                <h2 className="text-xl sm:text-2xl font-bold text-main">보유 자산 테이블</h2>
            </header>
            <div className="flex items-center gap-2 text-red-500">
                <AlertTriangle size={24} />
                <h3 className="text-lg font-semibold">보유 자산 테이블 정보 오류</h3>
            </div>
            <div className="text-center text-sm text-gray-600">
                <p>보유 자산 테이블 데이터 로딩 중 문제가 발생했습니다.</p>
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg flex items-center gap-2 text-gray-600">
                <Table size={16} />
                <span className="text-sm font-medium">보유 자산 테이블은 잠시 후 다시 시도해 주세요.</span>
            </div>
