...
interface RisedCoinsErrorFallbackProps {
    error: Error;
    resetErrorBoundary: () => void;
}

export function RisedCoinsErrorFallback({ error, resetErrorBoundary }: RisedCoinsErrorFallbackProps) {
    const handleRetry = () => {
        console.error("RisedCoins Error:", error);
        resetErrorBoundary();
    };
    ...
    return (
        <section className="contents-container flex flex-col gap-4">
            <header>
                <h2 className="text-xl sm:text-2xl font-bold text-main">상승률 TOP5</h2>
            </header>
            <div className="flex flex-col items-center justify-center py-8 gap-4">
                <div className="flex items-center gap-2 text-red-500">
                    <AlertTriangle size={24} />
                    <h3 className="text-lg font-semibold">상승률 정보 오류</h3>
                </div>
                <div className="text-center text-sm text-gray-600">
                    <p>상승률 데이터 로딩 중 문제가 발생했습니다.</p>
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-2 text-gray-600 mb-2">
                        <TrendingUp size={16} />
                        <span className="text-sm font-medium">참고 상승률 정보</span>
                    </div>
                    <div className="text-xs text-gray-500">
                        실시간 상승률 정보는 코인마켓캡, 업비트 등에서 확인하실 수 있습니다.
                    </div>
                </div>
